...
#myCounter1 is a version of the function with the which function, which() returns a list of the indexes
#of a list when given an identical list
myCounter1 <- function(number)
{
  result <- number + ( c <- which(number==number) )
  result
}

#another version of myCounter which just creates a vector of the same length and adds each value
#to the corresponding position, which in this case correspondes to the index of each element.
myCounter <- function(number)
{
  result <- number + ( c <- 1:length(number))
  result
}
myCounter(1)
